...
duplicateListRecursive:
     	...
     	move 	$t4, $a0
    	beq 	$t4, $0, baseCase
    	bne 	$t4, $0, copyHeadFirst
     
copyHeadFirst: 
   
    	move 	$t5, $a0
    	lw 	$t9, 4($t5)#get the data
    
    	li 	$a0, 8	# alocate 8 byte for new element, NEW NODE IS CREATED
    	li 	$v0, 9	# syscall for sbrk (dynamic memory allocation)
    	syscall
    
    	move 	$t6, $v0 #$t6 is the head of duplicated list, it will also be current pointer in the duplicated list
    	move 	$t7, $v0 #$t7 is head pointer of the duplicated list
    	sw 	$0,0($v0)	# node->next = null NEW NODE'S NEXT IS NULL
    	sw 	$t9,4($v0)	# node->number = number NEW NODE'S DATA IS ASSIGNED
    	lw 	$t5, 0($t5) #head = head-> next
    
    	j 	recursiveCase
    	
recursiveCase:
    	move 	$a0, $t5#recursive part begins here
    	addi 	$sp, $sp, -8
    	sw 	$a0,4($sp)
    	sw 	$ra,0($sp)
...
iterativeDuplicateDone:
	.asciiz "\n Linked list has been duplicated iteratively."
	
recursiveDuplicateDone:
	.asciiz "\n Linked list has been duplicated recursively."